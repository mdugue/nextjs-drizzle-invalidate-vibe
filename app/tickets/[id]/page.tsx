...
import { getMemberOptions, getProjectOptions, getTicketDetail } from "@/lib/queries";
...
export default async function TicketDetailPage({ params }: TicketDetailPageProps) {
  ...
  const projectLabel = projects.find((project) => project.id === ticket.projectId)?.title ?? "Unassigned";
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-8">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Created {formatDate(ticket.createdAt)} • Updated {formatDate(ticket.updatedAt)} • Project {projectLabel}
        </p>
      </div>
      ...
      <div className="rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="text-sm text-muted-foreground">Adjust status, assignee, and metadata.</p>
        <div className="mt-4">
          <TicketDetailForm
            ticket={ticket}
            projectOptions={projects.map((project) => ({ id: project.id, label: project.title }))}
            memberOptions={members.map((member) => ({ id: member.id, label: member.name }))}
          />
        </div>
      </div>
    </div>
