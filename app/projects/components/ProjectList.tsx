...
export function ProjectList({ projects, pageInfo, search, sort }: ProjectListProps) {
  const router = useRouter();
  const [sheetOpen, setSheetOpen] = React.useState(false);
  const [selected, setSelected] = React.useState<Project | null>(null);
  const [sortValue, setSortValue] = React.useState(sort ?? "createdAt");
  const [optimisticProjects, applyOptimistic] = useOptimistic(projects, (state, action: { type: "delete"; id: number }) => {
    ...
    <div className="flex flex-col gap-6">
      ...
      <form className="flex flex-wrap items-center gap-3" role="search">
        <div className="relative w-full max-w-sm">
          ...
          <Input name="search" placeholder="Search projects" defaultValue={search ?? ""} className="pl-10" />
        </div>
        ...
        {optimisticProjects.map((project) => (
          ...
          >
            <div className="space-y-1">
              ...
              {project.owner ? <p className="text-sm text-muted-foreground">Owned by {project.owner}</p> : null}
            </div>
