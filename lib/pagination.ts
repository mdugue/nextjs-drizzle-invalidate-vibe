...
export async function cursorPaginate<T, TCursor>(params: {
  ...
  const limit = Math.max(MIN_PAGE_SIZE, params.limit ?? DEFAULT_PAGE_LIMIT);
  ...
  const records = await fetcher({
    ...
    direction,
    limit: limit + EXTRA_RECORD_BUFFER,
  });
    ...
    hasNext:
      direction === "forward"
        ...
        : Boolean(params.cursor) ||
          records.length === limit + EXTRA_RECORD_BUFFER,
